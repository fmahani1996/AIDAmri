...
def main():
    ...
    if os.path.samefile(dir_path_in, dir_path_out):
    ...
    for index, item in enumerate(res_list):
        ...
        if item.endswith('.nii'):
            path_out = os.path.join(dir_path_out, ''.join([item, '.gz']))
        else:
            path_out = os.path.join(dir_path_out, item)
        rotate_data(path_in, path_out)
